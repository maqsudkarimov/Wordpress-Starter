...
class CPT {
     ...
     * Taxonomy add field
     *
     * @var array $taxonomy_add_field.
     */
    public $taxonomy_field;
    ...
    function __construct( $post_type_names, $options = array() ) {
        ...
        // Else the post type name is only supplied.
            ...
            // Set the create_posts.
            $this->create_posts = $this->get_create_posts();
        }
    ...
    function register_post_type() {
        ...
        $slug     = $this->slug;
        $create_posts = $this->create_posts;
     ...
     */
    function register_taxonomy($taxonomy_names, $options = array(), $taxonomy_add_form_field = false) {
        ...
        if ( is_array( $taxonomy_names ) ) {
            ...
            $singular = __( 'Category', 'qwerty' );
            //$plural   = $this->get_plural( $taxonomy_name );
            $plural   = __( 'Categories', 'qwerty' );
        ...
        if( $taxonomy_add_form_field ){
            
            $this->add_action( $this->taxonomy_name . '_add_form_fields', array( &$this, 'taxonomy_add_form_field' ), 10, 2 );
            $this->add_action( $this->taxonomy_name . '_edit_form_fields', array( &$this, 'taxonomy_edit_form_field' ), 10, 2 );
            $this->add_action( 'edited_' . $this->taxonomy_name, array( &$this, 'taxonomy_save_form_field' ), 10, 2 );  
            $this->add_action( 'create_' . $this->taxonomy_name, array( &$this, 'taxonomy_save_form_field' ), 10, 2 );
     ...
     *
     * @param array  $options Taxonomy field options.
     */


    function add_taxonomy_field( $options = array() ){

        if( isset( $options ) ) {
            $this->taxonomy_field = $options;
        }

    }

    function taxonomy_add_form_field(){

        foreach ( $this->taxonomy_field as $key => $value ) {
            if( $value['type'] == 'text' ){
    ?>
                <div class="form-field">
                    <label for="term_meta[<?php echo $value[0]; ?>]"><?php _e( $value['label'], $this->textdomain ); ?></label>
                    <input type="text" name="term_meta[<?php echo $value['name']; ?>]" id="term_meta[<?php echo $value['name']; ?>]">
                    <p class="description"><?php _e( $value['description'], $this->textdomain ); ?></p>
                </div>
    <?php
            } else { ?>
                ...
                <div class="form-field">
                    <label for="term_meta[<?php echo $value[0]; ?>]"><?php _e( $value['label'], $this->textdomain ); ?></label>
                    <input type="text" class="wpsa-url" name="term_meta[<?php echo $value['name']; ?>]" id="term_meta[<?php echo $value['name']; ?>]" value="" style="width:60%">
                    <input type="button" class="button wpsa-browse" value="Choose File" />
                    <img style="max-width:300px;border: 1px solid #ddd;margin-top: 5px;margin-left: 1px;display:block" src="" class="wpsa-image attachment-thumb">
                    <p class="description"><?php _e( $value['description'], $this->textdomain ); ?></p>
                </div>

      <?php }
        }
        $this->taxonomy_form_field_script();
    }

    function taxonomy_edit_form_field( $term ) {
    ...
    ?>
                <tr class="form-field">
                <th scope="row" valign="top"><label for="term_meta[<?php echo $value['name']; ?>]"><?php _e( $value['label'], $this->textdomain ); ?></label></th>
                    <td>
                        <input type="text" name="term_meta[<?php echo  $value['name']; ?>]" id="term_meta[<?php echo $value['name']; ?>]" value="<?php echo $term_meta[$value['name']]; ?>">
                        <p class="description"><?php _e( $value['description'], $this->textdomain ); ?></p>
                    </td>
                </tr>
    ...
    <?php
                ...
                <th scope="row" valign="top"><label for="term_meta[<?php echo $value['name']; ?>]"><?php _e( $value['label'], $this->textdomain ); ?></label></th>
                    <td>
                        <input type="text" class="wpsa-url" name="term_meta[<?php echo  $value['name']; ?>]" id="term_meta[<?php echo $value['name']; ?>]" value="<?php echo $term_meta[$value['name']]; ?>" style="width:80%">
                        <input type="button" class="button wpsa-browse" value="Choose File" />
                        <img style="max-width:300px;border: 1px solid #ddd;margin-top: 5px;margin-left: 1px;display:block" src="<?php echo $term_meta[$value['name']]; ?>" class="wpsa-image attachment-thumb">
                        <p class="description"><?php _e( $value['description'], $this->textdomain ); ?></p>
                    </td>
                </tr>
      <?php }
    ...
    function taxonomy_save_form_field( $term_id ) {
    ...
    function taxonomy_form_field_script(){
    ?>
        <script>
        jQuery(document).ready(function($) {
            $('.wpsa-browse').on('click', function (event) {
                ...
                file_frame.on('select', function () {
                    ...
                    self.prev('.wpsa-url').val(attachment.sizes.medium.url);
                    self.next('.wpsa-image').attr('src', attachment.sizes.medium.url);
                });
                file_frame.open();
            });
        });
